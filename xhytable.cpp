...
void xhytable::addfield(const xhyfield& field) {
    ...
    if (s.startsWith("check")||s.startsWith("CHECK")) {
        ...
        if (start > 0 && end > start) {  // 确保找到括号
            ...
            add_check_constraint(content,"");
            qDebug() << "[addfield] 字段" << field.name() << ": 已添加 CHECK 约束，条件为:" <<  content;
        } else {
...
bool xhytable::evaluateCheckExpression(const QString& expr, const QVariantMap& fieldValues) const {
    ...
    // 处理 IN 列表 (增强支持数字, 布尔, NULL, 字符串)
    QRegularExpression inClauseRe(R"(([\w.]+)\s+IN\s*\(\s*(.*?)\s*\))", QRegularExpression::CaseInsensitiveOption | QRegularExpression::DotMatchesEverythingOption);

    pos = 0; // Reset position for this loop
    while ((pos = jsExpr.indexOf(inClauseRe, pos)) != -1) {
        QRegularExpressionMatch match = inClauseRe.match(jsExpr, pos);
        QString field = match.captured(1);
        QString listContent = match.captured(2).trimmed(); // Get the content inside parens, trimmed

        QStringList jsValues; // List to hold JS literal strings (like "1", "'abc'", "true", "null")

        QRegularExpression itemRe(R"(\s*('[^']*'|\b-?\d+(\.\d+)?\b|\bTRUE\b|\bFALSE\b|\bNULL\b)\s*(?:,|$))", QRegularExpression::CaseInsensitiveOption);

        QRegularExpressionMatch itemMatch;
        int itemPos = 0;
        bool parseError = false;

        // Loop through the listContent to find each item
        while (itemPos < listContent.length()) {
            itemMatch = itemRe.match(listContent, itemPos);

            if (!itemMatch.hasMatch()) {
                // Failed to match an item at the current position - syntax error in list?
                parseError = true;
                break; // Exit inner loop
            }

            QString itemStr = itemMatch.captured(1); // The captured item's string value

            // Determine item type and convert to JS literal string
            if (itemStr.startsWith("'") && itemStr.endsWith("'")) {
                // It's a quoted string
                // Handle SQL '' escape
                QString unescapedContent = itemStr.mid(1, itemStr.length() - 2).replace("''", "'");
                // Format as a JS string literal (using single quotes and escaping internal single quotes)
                jsValues << "'" + unescapedContent.replace("'", "\\'") + "'";
            } else if (itemStr.compare("TRUE", Qt::CaseInsensitive) == 0) {
                jsValues << "true";
            } else if (itemStr.compare("FALSE", Qt::CaseInsensitive) == 0) {
                jsValues << "false";
            } else if (itemStr.compare("NULL", Qt::CaseInsensitive) == 0) {
                jsValues << "null";
            } else {
                jsValues << itemStr;
            }

            // Advance the position in listContent past the matched item and the separator (comma or end)
            itemPos = itemMatch.capturedEnd();
        }

        // Check if parsing the list content had errors or didn't consume the whole list
        if (parseError || (!listContent.trimmed().isEmpty() && itemPos < listContent.length())) {
            qWarning() << "无法完全解析 IN 列表内容，可能存在语法错误:" << listContent
                       << "在原始表达式中位置:" << match.capturedStart()
                       << "\n原始表达式:" << expr;
            // Decide how to handle: return false, or skip this IN clause?
            // Skipping seems safer to allow other parts of the expression to be checked.
            // Advance pos past the whole IN clause matched by inClauseRe
            pos = match.capturedEnd();
            continue; // Go to the next match attempt for inClauseRe
        }

        // Construct the JavaScript replacement string: "[item1, item2, ...].includes(field)"
        QString replacement = QString("[%1].includes(%2)").arg(jsValues.join(","), field);

        // Replace the original "field IN (...)" clause in the jsExpr string
        jsExpr.replace(match.capturedStart(), match.capturedLength(), replacement);

        // Adjust the outer loop's position by the length difference of the replacement
        // match.capturedLength() is the length of the original text replaced
        // replacement.length() is the length of the new text
        // pos needs to advance from where the replacement *ended* in the modified string
        pos += replacement.length() - match.capturedLength();
    }
...
bool xhytable::checkInsertConstraints(const QMap<QString, QString>& fieldValues) const {
    ...
    for (const QString& checkExpr : m_checkConstraints.values()) {
        if (!evaluateCheckExpression(checkExpr, fullRecordData)) {
            ...
            throw std::runtime_error("插入失败: 违反表级CHECK约束"+checkExpr.toStdString());
            return false;
        }
    }
